...
// import { generateAIResponse } from "@/lib/api/gemini-api" // Replaced by API call
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const apiRequestBody = {
        ...
        context,
        conversationId: conversationId, // Send current conversationId
      };
      ...
      // Assuming the backend returns a response structure like: { response: "AI message", conversationId: "newOrExistingId" }
      // Or if it directly returns the FastAPI structure: { message: "AI message", conversation_id: "id" }
      const aiContent = result.response || result.message;
  ...
  const clearConversation = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Error clearing conversation:", errorData.error);
        // Optionally show an error message to the user
        setMessages((prev) => [...prev, { role: "assistant", content: `Error clearing history: ${errorData.error || 'Unknown error'}` }]);
      } else {
        setMessages([
          {
            role: "assistant",
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: "I'm sorry, I encountered an error while clearing the conversation. Please try again later." }]);
    } finally {
      setIsLoading(false);
    }
  }
